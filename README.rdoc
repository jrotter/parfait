...
At the control layer, Parfait allows you to define Controls and Pages.  
...
Additionally, Regions can be defined on the page.  A Region is a subset of the page, targeted with specific data (for instance, a single entry in a list of names).  I'm still figuring out how Regions will work, so there's more to come in that space.

== How do I get started with Parfait?

=== Define a Page

To get started with Parfait, first we have to define a Page.  As an example, let's define a page that creates or edits a blog post.  Note that we'll handle other setup (like requires) in the workflow code later.  We'll put the page information in file +pages/edit_post.rb+.

  # Define the page - we'll use an alias since this is used to both add and edit
  edit_blog = Parfait.add_page(
    :name => "Edit Post",
    :aliases => ["New Post"])
